...
import { listVendorBills, createVendorBill, updateVendorBill, deleteVendorBill, type VendorBill, type CreateVendorBillRequest, type CreateLineItemRequest } from '@/services/vendorBills';
...
export default function VendorBillsPage() {
  ...
  const openEdit = (b: VendorBill) => {
    ...
    setForm({
      ...
      due_date: b.due_date || '',
      items: b.items.map((i) => ({ description: i.description, quantity: i.quantity, unit_price: i.unit_price })),
    });
    setDialogOpen(true);
  };
  ...
  const handleSave = async () => {
    try {
      ...
      if (editing) {
        await updateVendorBill(editing.id, payload);
      } else {
  ...
  return (
    <Box>
      <Flex justify="space-between" align="center" mb={4}>
        <Heading size="lg" color={"gray.100"}>Vendor Bills</Heading>
        <Button onClick={openCreate}><Plus size={16} /> Add Vendor Bill</Button>
      </Flex>
        ...
        <Table.Root variant="outline">
          ...
          <Table.Body>
            {bills.length > 0 ? bills.map((b) => (
              <Table.Row key={b.id}>
                <Table.Cell fontWeight="medium">{b.bill_number}</Table.Cell>
                <Table.Cell>{b.vendor_id ? vendors.find((v) => v.id === b.vendor_id)?.name || b.vendor_id : '-'}</Table.Cell>
                <Table.Cell>${Number(b.amount).toFixed(2)}</Table.Cell>
                <Table.Cell><Badge colorPalette={statusColors[b.status] || 'gray'}>{b.status}</Badge></Table.Cell>
                <Table.Cell>{b.due_date || '-'}</Table.Cell>
                <Table.Cell>
                  <Flex gap={2}>
                    <Button size="sm" variant="ghost" onClick={() => openEdit(b)}><Pencil size={14} /></Button>
                    <Button size="sm" variant="ghost" colorScheme="red" onClick={() => handleDelete(b.id)}><Trash2 size={14} /></Button>
                  </Flex>
                </Table.Cell>
              </Table.Row>
            )) : (
              <Table.Row><Table.Cell colSpan={6} textAlign="center" color="gray.500">No vendor bills found</Table.Cell></Table.Row>
            )}
          </Table.Body>
        </Table.Root>
      </Box>
      ...
      <Dialog.Root open={dialogOpen} onOpenChange={(e) => setDialogOpen(e.open)} size="lg">
        ...
        <Dialog.Positioner>
          <Dialog.Content>
            <Dialog.Header>{editing ? 'Edit Vendor Bill' : 'Add Vendor Bill'}</Dialog.Header>
            <Dialog.Body>
              <Stack gap={4}>
                <Field.Root required>
            ...
            <Dialog.Footer>
              ...
              <Button onClick={handleSave} disabled={!form.bill_number || form.items.length === 0}>{editing ? 'Update' : 'Create'}</Button>
            </Dialog.Footer>
          </Dialog.Content>
        </Dialog.Positioner>
      </Dialog.Root>
